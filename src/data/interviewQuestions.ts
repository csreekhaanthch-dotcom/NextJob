...
export const nodejsQuestions: InterviewQuestion[] = [
  ...
  },
  {
    ...
    answer: 'Middleware functions have access to req, res, and the next middleware function. They can execute code, modify req/res, end the response, or call next(). Middleware is executed in the order it\'s added.',
...
export const pythonQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'Lists are mutable (can be modified after creation), tuples are immutable (cannot be changed). Lists use square brackets [], tuples use parentheses (). Tuples are faster and can be used as dictionary keys.',
  ...
  },
  {
    ...
    answer: 'Decorators are functions that modify the behavior of other functions or classes. They wrap another function, allowing you to execute code before and after the wrapped function runs.',
  ...
  },
  {
    ...
    answer: 'Generators are functions that use yield to return values one at a time, pausing execution between yields. They maintain state between calls and generate values on-demand (lazy evaluation).',
    tips: 'Use generators when dealing with large datasets or infinite sequences to save memory.',
    sampleAnswer: 'def countdown(n):\n    while n > 0:\n        yield n\n        n -= 1\n\nfor i in countdown(5):\n    print(i)  # 5, 4, 3, 2, 1\n\n# Generator expression\nsquares = (x**2 for x in range(10))',
  ...
  },
  {
    ...
    answer: 'Comprehensions provide a concise way to create collections. List comprehensions create lists, dictionary comprehensions create dictionaries, and set comprehensions create sets.',
  ...
  },
  {
    ...
    answer: 'The GIL is a mutex that prevents multiple native threads from executing Python bytecodes simultaneously. It exists because CPython\'s memory management is not thread-safe.',
    tips: 'Use multiprocessing for CPU-bound tasks, threading for I/O-bound tasks.',
...
export const systemDesignQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'A URL shortener needs: 1) A hash function to generate short codes, 2) Database to store mappings, 3) Redirect service, 4) Analytics for tracking, 5) Caching for popular URLs, 6) Rate limiting.',
    tips: 'Consider trade-offs: hash collisions, length of short URL, custom aliases, analytics requirements.',
    sampleAnswer: 'Architecture:\n1. Client sends long URL to API Gateway\n2. Generate unique ID (Snowflake)\n3. Encode ID to base62\n4. Store in database + cache in Redis\n5. Redirect: Check cache -> DB -> 301 redirect',
  ...
  },
  {
    ...
    answer: 'Common algorithms: 1) Token Bucket, 2) Sliding Window, 3) Leaky Bucket. For distributed systems, use Redis with Lua scripts for atomic operations.',
    tips: 'Consider: rate limit per user/IP/API key, different limits for different endpoints, handling bursts.',
    sampleAnswer: '// Token Bucket with Redis\nconst rateLimit = async (userId, limit, window) => {\n  const key = `ratelimit:${userId}`;\n  // Lua script for atomic check-and-increment\n  // Return allowed/denied\n};',
  ...
  },
  {
    ...
    answer: 'A chat app needs: 1) WebSocket connections, 2) Message queue, 3) Database for persistence, 4) Push notifications, 5) End-to-end encryption, 6) Presence service.',
    tips: 'Consider message ordering, delivery receipts, read receipts, media handling, and cross-device sync.',
    sampleAnswer: 'Architecture:\n1. WebSocket servers for connections\n2. Message Queue (Kafka)\n3. Database (Cassandra/DynamoDB)\n4. Push notification service (FCM/APNs)\n5. Shard by chat_id',
  ...
  },
  {
    ...
    answer: 'A notification system needs: 1) Multiple channels (email, push, SMS, in-app), 2) Template engine, 3) Priority queue, 4) Retry mechanism, 5) Rate limiting, 6) Analytics.',
    tips: 'Consider deduplication, batching, user preferences, and handling failures gracefully.',
    sampleAnswer: 'Architecture:\n1. API Layer accepts requests\n2. Queue System (High/Medium/Low priority)\n3. Workers per channel\n4. Database for tracking\n5. Features: deduplication, batching, retry',
...
export const behavioralQuestions: InterviewQuestion[] = [
  {
    ...
    answer: 'Use the STAR method: Describe the Situation, your Task, the Action you took, and the Result. Focus on how you approached the situation professionally.',
    tips: 'Stay positive, focus on resolution, and show emotional intelligence. Never badmouth the difficult person.',
    sampleAnswer: 'S: Worked with a developer who consistently missed deadlines.\nT: Ensure project stays on track.\nA: Scheduled one-on-one, broke tasks into smaller chunks, paired with mentor.\nR: On-time delivery improved by 40%.',
  ...
  },
  {
    ...
    answer: 'Choose a project with clear challenges and measurable outcomes. Explain the technical or business challenges, your approach, and the positive impact.',
    tips: 'Focus on YOUR contributions, not just the team\'s. Quantify results when possible.',
    sampleAnswer: 'S: Migrate monolith to microservices with zero downtime.\nT: Create migration strategy.\nA: Strangler fig pattern, canary deployments, monitoring.\nR: 99.9% uptime, 60% faster response time.',
  ...
  },
  {
    ...
    answer: 'Choose a real mistake, take ownership, explain what you learned, and show how you prevented similar issues.',
    tips: 'Be honest but not overly negative. Focus on learning and prevention.',
    sampleAnswer: 'S: Pushed DB migration to production without testing, caused 30-min outage.\nT: Fix and prevent.\nA: Rolled back, communicated, created deployment checklist.\nR: Deployment issues reduced by 80%.',
  ...
  },
  {
    ...
    answer: 'Show your learning process, resources you used, and how you applied the new knowledge.',
    tips: 'Show structured learning approach and practical application.',
    sampleAnswer: 'S: Team needed WebSockets feature in 3 days.\nT: Learn and implement.\nA: Read docs, built POC, watched talks, documented.\nR: Delivered on time, 10K concurrent connections.',
  ...
  },
  {
    ...
    answer: 'Describe your time management strategies, prioritization methods, and how you maintain quality under pressure.',
    tips: 'Mention specific tools or methods (Pomodoro, Eisenhower Matrix).',
    sampleAnswer: '1. Prioritize ruthlessly (Eisenhower Matrix)\n2. Communicate early\n3. Break down tasks\n4. Block deep work hours\n5. Maintain quality gates',
