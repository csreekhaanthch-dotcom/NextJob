...
import { 
  ...
  Bell, BellRing, CheckCircle, XCircle, AlertTriangle,
...
export default function NextJobPlatform() {
  ...
  const [selectedInterviewJob, setSelectedInterviewJob] = useState<any>(null)
  ...
  const [newAlert, setNewAlert] = useState<any>({ name: '', search_query: '', frequency: 'daily' })
  ...
  const toggleSaveJob = (job: any) => {
    const isSaved = savedJobs.some((j: any) => j.id === job.id)
    const updated = isSaved ? savedJobs.filter((j: any) => j.id !== job.id) : [...savedJobs, job]
  ...
  const generateInterviewQuestions = async (job: any) => {
  ...
  const fetchCompanyIntel = async (companyName: string) => {
    ...
    try {
      ...
      const data = await response.json()
      setCompanyIntel(data && data.avg_rating ? data : {
        company: companyName,
        ...
        recent_reviews: [{ job_title: 'Software Engineer', rating: 4, pros: 'Great culture', cons: 'Long hours' }]
      })
    } catch (error) {
      console.error('Error:', error)
    } finally {
  ...
  const toggleAlert = (alertId: string) => {
    const updated = jobAlerts.map((a: any) => a.id === alertId ? { ...a, is_active: !a.is_active } : a)
    setJobAlerts(updated)
    localStorage.setItem('jobAlerts', JSON.stringify(updated))
  }

  const deleteAlert = (alertId: string) => {
    const updated = jobAlerts.filter((a: any) => a.id !== alertId)
    setJobAlerts(updated)
    localStorage.setItem('jobAlerts', JSON.stringify(updated))
  }
  ...
  const getMatchColor = (score: number) => score >= 80 ? 'text-green-600 bg-green-50' : score >= 60 ? 'text-yellow-600 bg-yellow-50' : 'text-red-600 bg-red-50'
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Select value={activeTab} onValueChange={setActiveTab}>
                ...
                <SelectContent>
                  <SelectItem value="jobs">Find Jobs</SelectItem>
                  <SelectItem value="resume">Resume AI</SelectItem>
                  <SelectItem value="interview">Interview</SelectItem>
                  <SelectItem value="intel">Company Intel</SelectItem>
                  ...
                  <CardContent className="p-5">
                    ...
                    <div className="flex items-center gap-2 pt-4 border-t">
                      <Button variant="ghost" size="sm" onClick={() => toggleSaveJob(job)}>
                        {savedJobs.some((j: any) => j.id === job.id) ? <BookmarkCheck className="w-4 h-4 text-blue-600" /> : <Bookmark className="w-4 h-4" />}
                      </Button>
                      <Dialog>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="space-y-6">
                  ...
                  <div><h4 className="font-semibold flex items-center gap-2 mb-2"><CheckCircle className="w-4 h-4 text-green-600" />Strengths</h4><ul className="space-y-1">{resumeAnalysis.strengths.map((s: string, i: number) => <li key={i} className="text-sm text-gray-600">• {s}</li>)}</ul></div>
                  <div><h4 className="font-semibold flex items-center gap-2 mb-2"><XCircle className="w-4 h-4 text-red-600" />Improve</h4><ul className="space-y-1">{resumeAnalysis.weaknesses.map((w: string, i: number) => <li key={i} className="text-sm text-gray-600">• {w}</li>)}</ul></div>
                  <div><h4 className="font-semibold flex items-center gap-2 mb-2"><Target className="w-4 h-4 text-blue-600" />Skills</h4><div className="flex flex-wrap gap-2">{resumeAnalysis.skills_detected.map((s: string, i: number) => <Badge key={i} variant="outline">{s}</Badge>)}</div></div>
                </div>}
              </CardContent>
            </Card>
          </div>
        ...
        {activeTab === 'interview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card>
              ...
              <CardContent>
                {savedJobs.length > 0 ? <div className="space-y-2">{savedJobs.map((job: any) => (
                ...
                ))}</div> : <div className="space-y-2"><Input placeholder="Job Title" value={selectedInterviewJob?.title || ''} onChange={(e) => setSelectedInterviewJob({ ...selectedInterviewJob, id: 'manual', title: e.target.value } as any)} /><Input placeholder="Company" value={selectedInterviewJob?.company || ''} onChange={(e) => setSelectedInterviewJob({ ...selectedInterviewJob, id: 'manual', title: selectedInterviewJob?.title || '', company: e.target.value } as any)} /></div>}
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                ...
                <div className="space-y-4">{interviewQuestions.map((q: any, i: number) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div><Label>Name</Label><Input placeholder="e.g., React Jobs" value={newAlert.name || ''} onChange={(e) => setNewAlert((prev: any) => ({ ...prev, name: e.target.value }))} /></div>
                  <div><Label>Search Query</Label><Input placeholder="e.g., React, TypeScript" value={newAlert.search_query || ''} onChange={(e) => setNewAlert((prev: any) => ({ ...prev, search_query: e.target.value }))} /></div>
                  <div><Label>Frequency</Label><Select value={newAlert.frequency} onValueChange={(v) => setNewAlert((prev: any) => ({ ...prev, frequency: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="daily">Daily</SelectItem><SelectItem value="weekly">Weekly</SelectItem></SelectContent></Select></div>
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                ...
                <div className="space-y-4">{jobAlerts.map((alert: any) => (
        ...
        {activeTab === 'dashboard' && (
          <div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              ...
              <Card><CardContent className="p-6"><div className="text-3xl font-bold text-purple-600">{applications.filter((a: any) => a.status === 'interviewing').length}</div><p className="text-sm text-gray-500">Interviews</p></CardContent></Card>
              <Card><CardContent className="p-6"><div className="text-3xl font-bold text-amber-600">{jobAlerts.filter((a: any) => a.is_active).length}</div><p className="text-sm text-gray-500">Active Alerts</p></CardContent></Card>
            </div>
            <Card>
              ...
              <CardContent>
                ...
                <div className="space-y-4">{savedJobs.map((job: any) => (
      ...
      <Dialog open={showAlertDialog} onOpenChange={setShowAlertDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>Name</Label><Input value={newAlert.name} onChange={(e) => setNewAlert((prev: any) => ({ ...prev, name: e.target.value }))} /></div>
            <div><Label>Query</Label><Input value={newAlert.search_query} onChange={(e) => setNewAlert((prev: any) => ({ ...prev, search_query: e.target.value }))} /></div>
            <div><Label>Frequency</Label><Select value={newAlert.frequency} onValueChange={(v) => setNewAlert((prev: any) => ({ ...prev, frequency: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="daily">Daily</SelectItem><SelectItem value="weekly">Weekly</SelectItem></SelectContent></Select></div>
          </div>
