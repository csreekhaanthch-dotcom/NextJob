...
function estimateExperience(text: string): number {
  ...
  if (dateRanges.length > 0) {
    // Each range roughly represents 2-3 years on average
...
function extractCompanies(text: string): string[] {
  // This is challenging without context, but we can try some patterns
...
function calculateATSScore(text: string): number {
  ...
  // Check for keywords matching job requirements
...
function identifyATSIssues(text: string): string[] {
  ...
  if (!/skills?/i.test(text)) {
    issues.push('No clear skills section')
  }
  if (!/experience|employment|work history/i.test(text)) {
    issues.push('No clear experience section')
  }
  ...
  const skills = extractTechnicalSkills(text)
  if (skills.length < 5) {
    issues.push('Few technical keywords detected')
  }
...
export function calculateJobMatch(resumeText: string, jobDescription: string): {
  ...
  if (jobSkills.length > 0) {
    ...
    score += 20 // If no skills detected in job, give partial credit
  }
